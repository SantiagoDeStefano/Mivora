...
export default function NavHeader({ user}: NavHeaderProps) {
  // state để biết ảnh có lỗi hay không -> fallback sang initials
  const [imgError, setImgError] = useState(false);

  // url ưu tiên: user.avatarUrl; nếu không có thì dùng url bạn cung cấp
  const avatarUrl =
    user?.avatarUrl ||
        ...
        <div className="flex items-center gap-2">
          ...
          <details className="relative group">
            ...
            >
              ...
              <Link
                to="/users/logout"
                className="block px-3 py-2 rounded-lg text-sm text-gray-700 dark:text-pink-500 hover:bg-gray-100 dark:hover:bg-gray-800"
